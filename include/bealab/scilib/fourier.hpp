...
template<class T>
sequence<T> time_shift( sequence<T> X, int t )
{
    X.t1( X.t1()+t );
    return X;
}
...
template<class T>
sequence<decltype(T()*complex(0,0))> frequency_shift( const sequence<T>& X, double w )
{
    // Modulating function
...
/// DTFT of a sequence
template<class T>
function<decltype(T()*complex(0))(double)> dtft( const sequence<T>& x )
{
	typedef decltype(T()*complex(0)) R;
	return [x]( double w ) -> R
	{
		ivec t = vrange( x.t1(), x.t2()+1 );
		return sum( element_prod( x.buffer(), exp(-j*w*t) ) );
	};
}

/// DTFT of a sequence evaluated at a vector of frequencies.
template<class T>
vec<decltype(T()*complex(0))> dtft( const sequence<T>& X, const rvec& W )
{
	return entrywise( dtft(X), W );
}
//template<class T>
//vec<decltype(T()*complex())> dtft( const sequence<T>& X, const rvec& W )
//{
//    int I = W.size();
...
template<class T>
vec<decltype(T()*complex(0))> dtft( sequence<T> X, int M )
{
    int L = X.size();
    ...
    sequence<T> Xt = X.trunc( X.t1(), N+X.t1()-1 );
    vec<T> v       = circshift( Xt.buffer(), X.t1() );
    vec<R> fv      = dft( v );
    ivec idxs      = K * vrange( 0, M );
    return fv( indirect( idxs ) );
}
...
template<class T>
sequence<decltype(T()*complex(0))> idtft( const vec<T>& X, int t1 )
{
    typedef decltype(T()*complex(0)) R;
    const vec<R>& Yo = idft( X );
    const vec<R>& Y  = circshift( Yo, -t1 );
    return sequence<R>( Y, t1 );
}
...
template<class T>
sequence<decltype(T()*complex(0))> idtft(const vec<T>& X )
{
